...
public class IntelligentBeautify {
	...
	public static Bitmap apply(Context cx, Bitmap bm, int enlarge, int thin,
			int skin) {
		int w = bm.getWidth();
		int h = bm.getHeight();
		...
		if (result.humans.length > 0) {
			...
			partialFaceProcess(pixels, w, h, human.leftEye, human.rightEye, human.mouth, enlarge, thin);
		}
		SmoothSkin sk = new SmoothSkin();
		sk.setup(cx, bm);
		bm = sk.apply(cx, skin);
		sk.release();
		return bm;
