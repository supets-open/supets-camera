...
public class RSCameraG3Live extends RSLineFilter {
	...
	public boolean apply(Context cx, String inPath, String outPath, int[] args) {
		mCurve = new Curve(cx, "curves/live_g3.dat");
		mLayer = Layer.getLayerImage(cx, "layers/live_g3", Layer.Type.NORMAL);
	...
	protected void applyLine(Context cx, int[] pixels, int line, int height) {
		...
		CMTProcessor.rsLinearBurn(pixels, layerPixels, w, 1, mLayer.getWidth(), 1, 60);
	}
