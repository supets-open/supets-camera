...
public class RSRotate extends RealsizeFilter {
	...
	public boolean apply(Context cx, String inPath, String outPath, int[] args) {
		...
		for (currentColumn = 0; currentColumn+stepSpan <= wSrc; currentColumn+=stepSpan) {
			...
			pixelRect = getRotatedPixelRect(0, hSrc, currentColumn, currentColumn+stepSpan);
		...
		if (currentColumn < wSrc) {
			...
			pixelRect = getRotatedPixelRect(0, hSrc, currentColumn, wSrc);
			JpegSupport.writeJpegLines(pixelRect, wSrc-currentColumn);
			JpegSupport.finishReadingAndRelease();
			needInit = true;
		}
