...
public class RSCameraLomoLive extends RSLineFilter {
	...
	public boolean apply(Context cx, String inPath, String outPath, int[] args) {
		mCurve = new Curve(cx, "curves/live_lomo.dat");
		mLayer = Layer.getLayerImage(cx, "layers/live_lomo", Layer.Type.NORMAL);
	...
	protected void applyLine(Context cx, int[] pixels, int line, int height) {
		...
		ImageProcessUtils.saturationPs(pixels, w, 1, -10);
		...
		CMTProcessor.rsLinearBurn(pixels, layerPixels, w, 1, mLayer.getWidth(), 1, 40);
	}
