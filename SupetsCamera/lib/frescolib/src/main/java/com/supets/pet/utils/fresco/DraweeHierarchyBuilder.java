...
public class DraweeHierarchyBuilder {
    ...
    public static GenericDraweeHierarchy buildRoundDraweeHierarchys(int imageId, boolean pressable, ImageView imageView) {
        ...
        if(existed == null) {
            ...
            if (imageId != -1) {
                builder.setPlaceholderImage(ResourcesCompat.getDrawable(context.getResources(),imageId,null));
                builder.setFailureImage(ResourcesCompat.getDrawable(context.getResources(),imageId,null));
            }

            if (pressable) {
                builder.setPressedStateOverlay(
                        ResourcesCompat.getDrawable(context.getResources(),R.drawable.shape_header_press_overlay_default,null));
            }
