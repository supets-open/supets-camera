 ...
 *
 * <p>
 *     提供一个{@link Builder}类来构建SpannableString
 * </p>
 *
...
public class SpanUtils {
    ...
    public static SpannableString MultiMatch(String content, @ColorRes int colorRes) {
        ...
        while (matcher.find()) {
            groupCount++;
            spannableString.setSpan(new ForegroundColorSpan(ContextCompat.getColor(UIUtils.getContext(), colorRes)),
                    matcher.start() - 2 * (groupCount - 1), matcher.end() - 2 * groupCount, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        return spannableString;
    }
